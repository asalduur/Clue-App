...
export const SocketProvider = (props) => {
  ...
  const [fakeplayer, setFakePlayer] = useState({player: 'Player 1', 
    ...
    if (socket) {
      ...
      socket.on('room-choose', (a_player) => {
        console.log(a_player);
            if (playerRef.current.id === a_player.id) {
              ...
              setInactiveMsg(`Player ${a_player.player} is currently choosing a room.`);
            }});
      socket.on('accuse-suggest', (a_player) => {
        if (playerRef.current.id === a_player.id) {
          ...
          setInactiveMsg(`Player ${a_player.player} is currently making a suggestion or accusation.`)
        }
      })
      ...
      socket.on('player-lost', (data) => {
        const {body} = data;
        setLossMsg(`You lost while accusing the ${body.suspect} of murdering Mr. Boddy in the ${body.room} with a ${body.weapon}!`);
        setInactiveMsg(`Player ${data.player} lost while accusing the ${body.suspect} of murdering Mr. Boddy in the ${body.room} with a ${body.weapon}!`);
        setTimeout(() => {
      ...
      socket.on('game-win', (data) => {
        const {body} = data;
        setInactiveMsg(`Player ${body[1].player} accused ${body[0].suspect} of murdering Mr. Boddy in the ${body[0].room} with a ${body[0].weapon}, and WON!`);
        setWinMsg(`You accused ${body[0].suspect} of murdering Mr. Boddy in the ${body[0].room} with a ${body[0].weapon}, and WON!`);
        setTimeout(() => {
          if (body.id !== socket.id) {
            setPlayerLost(true);
          } else {
          ...
          setResetMsg(`Player ${body[1].player} has won and the game has ended. Please refresh your browser to join a new game.`)
      ...
      socket.on('suggest-message', (data) => {
        ...
        if (playerRef.current.id !== activeplayer.id) {
          setInactiveMsg(`Player ${activeplayer.player} is suggesting that the ${body.suspect} killed Mr. Boddy in the ${body.room} with a ${body.weapon}.`);
        }
      })
      socket.on('accuse-choice', (player) => {
        if (playerRef.current.id === player.id) {
          ...
          setInactiveMsg(`No one could prove the suggestion wrong. Player ${player.player} is deciding whether or not to make an accusation.`)
        }
      })
