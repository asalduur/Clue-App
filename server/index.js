...
const cards = [[
    {weapon: 'Poisoned Cocktail'}, {weapon: 'Shank'}, {weapon: 'Coat Hanger'}, 
    {weapon: 'Chainsaw'}, {weapon: 'Blowtorch'}, {weapon: 'Ninja Star'}],
  [
...
io.on('connection', (socket) => {
  ...
  switch (players.length) {
    case 0:
      players.push({player: 1, id: socket.id, location: 'home', 
                    roll: 0, token: tokens.splice(0, 1)});
                    console.log(players);
      break;
    case 1:
      players.push({player: 2, id: socket.id, location: 'home', 
                    roll: 0, token: tokens.splice(0, 1)});
                    console.log(players);
      break;
    case 2:
      players.push({player: 3, id: socket.id, location: 'home', 
                    roll: 0, token: tokens.splice(0, 1)});
                    console.log(players);
      break;
    case 3:
      players.push({player: 4, id: socket.id, location: 'home', 
                    roll: 0, token: tokens.splice(0, 1)});
                    console.log(players);
      break;
    default:
  ...
  socket.on('disconnect', (body) => {
    ...
    if (players.length >= 3) {
      ...
      io.emit('player-disconnected', 'A player disconnected and the game has ended. Please refresh to start a new game.');
      io.disconnectSockets();
      players = [];
      tokens = ['blue', 'red', 'green', 'yellow'];
    }
  ...
  socket.on('game-start', async () => {
    ...
    playercopy.sort(function() { return 0.5 - Math.random() });

    let weaponArr = [...cards[0]];
    let roomArr = [...cards[1]];
    let peopleArr = [...cards[2]];
    const [murder_weapon] = weaponArr.splice(Math.floor(Math.random() * weaponArr.length), 1);
    const [murder_perp] = peopleArr.splice(Math.floor(Math.random() * peopleArr.length), 1);
    const [murder_room] = roomArr.splice(Math.floor(Math.random() * roomArr.length), 1);
  ...
  socket.on('send-suggestion', (body) => {
    ...
    othercards.sort(function() { return 0.5 - Math.random() });
